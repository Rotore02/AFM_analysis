...
def build_image_correction_pipeline(settings, results_file):
    ...
    -------
    list
        list of the functions to be executed based on the `settings` file.

    Raises
    ------
    TypeError
        If the plane subtraction keyword in the `settings` file is not valid.
    TypeError
        If the line subtraction keyword in the `settings` file is not valid.
    TypeError
        If the data shift keyword in the `settings` file is not valid.
    UserWarning
        If any kind of line drift correction is requested without plane subtraction.

    See Also
    --------
    image_correction_functions: module with all the image correcting functions.
