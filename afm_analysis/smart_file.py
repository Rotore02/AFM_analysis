class SmartFile:
    ...
    This class can be used to create a file-like object in which an 
    ...
    def __init__(self):
        """
        Initializes the SmartFile object with `enabled = False` and 
    ...
    def write(self, message):
        """
        Writes the string `message` in the file only if `enabled` 
        is set to `True` and `file` is not `None`.
    ...
    def close(self):
        """
        Closes the file object if `enabled` is set to `True` and 
        `file` is not `None`.
        """
        if self.enabled and self.file:
            self.file.close()
