...
def create_coordinate_grid(
    ...
    image_width : float # Dimensions: [m], is set manually in the AFM
) -> tuple[np.ndarray,np.ndarray]:
    """
    Creates the coordinate grid in real space.

    Takes as inputs the scanning rate [m^(-1)] and the image width
    ...
    coordinate corresponds an height value.
...
def plot_2d_image(
    ...
    height_values : np.ndarray, # 2-d array
    coordinate_grid : tuple[np.ndarray, np.ndarray], # Couple of 2-d arrays
    color_map='Greys'
) -> None:
    """
    Plots AFM data as a 2D color map.

    Makes a 2-d plot of the height values stored in the 2-d grid 
    labeling the x and y axis of the plot with the coordinates 
    stored in `coordinate_grid`.
    ...
    See Also
    --------
    plt.colormaps: register with a list of all colormaps aviable from 
    matplotlib.
    """
    if color_map not in plt.colormaps():
        raise TypeError(
            "The inserted color map does not exist. " \
            "You can find all the valid color maps at " \
            "https://matplotlib.org/stable/users/explain/colors/colormaps.html"
        )
    
    if (not coordinate_grid[0].shape == height_values.shape
        or not coordinate_grid[1] .shape == height_values.shape):
        raise ValueError(
            "Coordinate matrix and height values array have " \
            "different shapes. Please check that the scanning rate"
            " and the image length are inserted correctly."
        )
    ...
    im = ax.imshow(
        height_values, extent=real_space_map, cmap=color_map, aspect='auto'
    )
    ax.set_xlabel('X (µm)')
    ax.set_ylabel('Y (µm)')
...
def plot_3d_image(
    ...
    Makes a 3-d plot of the height values stored in the 2-d grid 
    labeling the x and y axis of the plot with the coordinates 
    stored in `coordinate_grid`.
    ...
    See Also
    --------
    plt.colormaps: register with a list of all colormaps aviable 
    from matplotlib.
    """
    if color_map not in plt.colormaps():
        raise TypeError(
    ...
    if (not coordinate_grid[0].shape == height_values.shape 
        ...
        raise ValueError(
            "Coordinate matrix and height values array have " \
            "different shapes. Please check that the scanning " \
            "rate and the image length are inserted correctly."
        )
...
def custom_plot(
    ...
    This function generates a plot of the data stored in `data`, 
    where `data[0]` is the x axis and `data[1]` is the y axis.
